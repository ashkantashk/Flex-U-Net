...
class FlexUnet():
  def __init__(self):
    self.IMG_HEIGHT, self.IMG_WIDTH, self.IMG_CHANNELS = [256, 256, 3]
    self.inputs_size = self.input_size = (self.IMG_HEIGHT, self.IMG_WIDTH, self.IMG_CHANNELS)

  ## Flex-U-Net

  def Con_Exp_Path(self, X, Y, n):
  ...
  def Flex_U_Net_Segmentation(self, NN, KK, dd, Final_Act, backbone=None):    
      if backbone==None:
          ...
          for tt in np.arange(dd - 1, -1, -1):
              ...
              MD[gg + 3 * (dd - tt - 1) + 3] = self.Con_Exp_Path(NN + KK - (dd - tt), 0.1 * (1 + np.fix((KK - dd + tt) / 2)), \
                                                            MD[gg + 3 * (dd - tt - 1) + 2])
          ...
          for tt in np.arange(0, dd):
              ...
              MD[gg + 2 * tt + 2] = self.Con_Exp_Path(NN + KK + (tt + 1 - dd), 0.1 * (1 + np.fix((KK - dd + tt + 1) / 2)), \
                                                  MD[gg + 2 * tt + 1])
          ...
          for tt in np.arange(KK, dd, -1):
              ...
              MD[gg + 3 * (KK - tt) + 3] = self.Con_Exp_Path(NN + tt - (dd + 1), 0.1 * (1 + np.fix((tt - dd - 1) / 2)), \
                                                        MD[gg + 3 * (KK - tt) + 2])
          if gg != NL - 1:
              gg += 3 * (KK - dd)
          ...
          if backbone =='vgg16':
              ...
              if KK==4 and dd==4:
                  ...
                  #### Sub-Expanding Path
                  se1=self.decoder_block(c5,c4,512)
                  se2=self.decoder_block(se1,c3,256)
                  se3=self.decoder_block(se2,c2,128)
                  se4=self.decoder_block(se3,c1,64)
                  ...
                  #### Expanding Path
                  e1=self.decoder_block(sc4,sc3,512)
                  e2=self.decoder_block(e1,sc2,256)
                  e3=self.decoder_block(e2,sc1,128)
                  e4=self.decoder_block(e3,se4,64)               
                  ...
                  #### Expanding Path
                  e1=self.decoder_block(sc3,sc2,512)
                  e2=self.decoder_block(e1,sc1,256)
                  e3=self.decoder_block(e2,se3,128)
                  e4=self.decoder_block(e3,c1,64)               
                  dropout = Dropout(0.8)(e4)
              elif KK==4 and dd==2:
                  #### Contracting Path
                  c1=vgg16.layers[1](xx)
                  ...
                  #### Expanding Path
                  e1=self.decoder_block(sc2,sc1,512)
                  e2=self.decoder_block(e1,se2,256)
                  e3=self.decoder_block(e2,c2,128)
                  e4=self.decoder_block(e3,c1,64)               
                  dropout = Dropout(0.8)(e4)
              elif KK==4 and dd==1:
                   #### Contracting Path
                  ...
                  #### Expanding Path
                  e1=self.decoder_block(sc1,se1,512)
                  e2=self.decoder_block(e1,c3,256)
                  e3=self.decoder_block(e2,c2,128)
                  e4=self.decoder_block(e3,c1,64)               
                  dropout = Dropout(0.8)(e4)
              elif KK==3 and dd==3:
                  ...
                  c4=vgg16.layers[13](c4)
                  #### Sub-Expanding Path
                  se1=self.decoder_block(c4,c3,256)
                  se2=self.decoder_block(se1,c2,128)
                  se3=self.decoder_block(se2,c1,64)                
                  #### Sub-Contracting Path
                  ...
                  sc3=vgg16.layers[13](sc3)                
                  #### Expanding Path
                  e1=self.decoder_block(sc3,sc2,256)
                  e2=self.decoder_block(e1,sc1,128)
                  e3=self.decoder_block(e2,se3,64)
                  ...
                  #### Sub-Expanding Path
                  se1=self.decoder_block(c4,c3,256)  # 64x128x256
                  se2=self.decoder_block(se1,c2,128) # 128x256x128                  
                  #### Sub-Contracting Path
                  ...
                  #### Expanding Path
                  e1=self.decoder_block(sc2,sc1,256)
                  e2=self.decoder_block(e1,se2,128)
                  e3=self.decoder_block(e2,c1,64)
                  dropout = Dropout(0.8)(e3)
              elif KK==3 and dd==1:
                  #### Contracting Path
                  c1=vgg16.layers[1](xx)
                  ...
                  #### Expanding Path
                  e1=self.decoder_block(sc1,se1,256)
                  e2=self.decoder_block(e1,c2,128)
                  e3=self.decoder_block(e2,c1,64)
                  dropout = Dropout(0.8)(e3)
              elif KK==2 and dd==2:
                  #### Contracting Path
                  ...
                  c3=vgg16.layers[9](c3) # 64x128x256
                  #### Sub-Expanding Path
                  se1=self.decoder_block(c3,c2,128)  # 128x256x128
                  se2=self.decoder_block(se1,c1,64) # 256x512x64
                  #### Sub-Contracting Path
                  ...
                  #### Expanding Path
                  e1=self.decoder_block(sc2,sc1,128) # 
                  e2=self.decoder_block(e1,se2,64)
                  ...
                  sc1=vgg16.layers[9](sc1) # 64x128x256
                  #### Expanding Path
                  e1=self.decoder_block(sc1,se1,128) # 128x256x128
                  e2=self.decoder_block(e1,c1,64) # 256x512x64
                  dropout = Dropout(0.8)(e2)                
              """ Output """
